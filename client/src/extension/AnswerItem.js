            ...
            <React.Fragment>
                <div style={styles.itemContainer}>
                    <div style={styles.textContainer}>
                        ...
                        <p> Animal elements: {animal.value.map((a, i) => {return <React.Fragment key={a}>{a}{i===animal.value.length-1 ? '' : ','} </React.Fragment>})} </p>
                        <p> Other animal elements: {animalOther.value} </p>
                        <p> Vegetation elements: {vegetation.value.map((v, i) => {return <React.Fragment key={v}>{v}{i===vegetation.value.length-1 ? '' : ','} </React.Fragment>})} </p>
                        <p> Other vegetation elements: {vegetationOther.value} </p>
                        <p> Man made elements: {manmade.value.map((mm, i) => {return <React.Fragment key={mm}>{mm}{i===manmade.value.length-1 ? '' : ','} </React.Fragment>})} </p>
                        <p> Other man made elements: {manmadeOther.value} </p>
                        <p> Motivation: {motivation.value.map((m, i) => {return <React.Fragment key={m}>{m}{i===motivation.value.length-1 ? '' : ','} </React.Fragment>})} </p>
